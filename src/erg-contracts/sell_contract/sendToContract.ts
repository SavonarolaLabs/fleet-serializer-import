...
import { SByte, SColl, SGroupElement, SLong, SSigmaProp } from "@fleet-sdk/serializer";
...
export function sellForTokenTx(contract: string, sellerBase58PK: string, utxos: Array<any>, height: number,assets:Array<any>, currencyAmount: bigint, currencyTokenId: string): any{
    const myAddr = ErgoAddress.fromBase58(sellerBase58PK)
    const output = new OutputBuilder(
        SAFE_MIN_BOX_VALUE + RECOMMENDED_MIN_FEE_VALUE,
        contract
    ...
    .setAdditionalRegisters({
        R4: SSigmaProp(SGroupElement(first(myAddr.getPublicKeys()))).toHex(),
        R5: SLong(currencyAmount).toHex(),
        R6: SColl(SByte, currencyTokenId).toHex(), 
    });
